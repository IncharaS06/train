...
export default function MaterialsDetailsClient() {
  ...
  async function loadMaterial(materialId: string) {
    try {
      ...
      if (!snap.exists()) {
        ...
        router.push("/track/installation"); // (You can change to /track/materials if needed)
        return;
      }
                ...
                {editing && !saving && (
                  <button
                    ...
                    onClick={() => {
                      ...
                      setJioTagPhoto(null);
                      if (data.jioTagPhotoUrl)
                        setJioTagPreview(data.jioTagPhotoUrl);
                    }}
